...
logistic_fit <- fit(logistic_mod,
                    factor(vote) ~ ideology + gender + race,
                    data = nes)
logistic_fit$fit %>%
  tidy(conf.int = TRUE) %>%
...
vote_tree <- fit(tree_mod,
                 vote ~ ideology + gender + race + income,
                 data = nes)
...
vote_tree <- fit(update(tree_mod, cost_complexity = 0.0),
...
tibble(`Error rate` = vote_forest$fit$err.rate[, "OOB"],
       Trees = 1:500) %>%
